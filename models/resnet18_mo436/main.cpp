...
bool readPngImage(const char *filename, float *&imageT, size_t *imageDims) {
    ...
    // normalization specific to resnet18 model
    float scale = 255;
    ...
    for (size_t row_n = 0; row_n < height; row_n++) {
        ...
        for (size_t col_n = 0; col_n < width; col_n++) {
            ...
            for (size_t ch = 0; ch < numChannels; ch++) {
                imageT[getXYZ(imageDims, row_n, col_n, ch)] = ((float)(ptr[ch] / scale) - mean[ch]) / sdev[ch];
            }
        }
    }
...
/// initialize.
GLOW_MEM_ALIGN(RESNET18_MEM_ALIGN)
uint8_t constantWeight[RESNET18_CONSTANT_MEM_SIZE] = {
#include "resnet18.weights.txt"
};

/// Statically allocate memory for mutable weights (model input/output data).
GLOW_MEM_ALIGN(RESNET18_MEM_ALIGN)
uint8_t mutableWeight[RESNET18_MUTABLE_MEM_SIZE];

/// Statically allocate memory for activations (model intermediate results).
GLOW_MEM_ALIGN(RESNET18_MEM_ALIGN)
uint8_t activations[RESNET18_ACTIVATIONS_MEM_SIZE];

/// Bundle input data absolute address.
uint8_t *inputAddr = GLOW_GET_ADDR(mutableWeight, RESNET18_data);

/// Bundle output data absolute address.
uint8_t *outputAddr = GLOW_GET_ADDR(mutableWeight, RESNET18_resnetv15_dense0_fwd__1);
