...
/// This example loads and runs the compiled squeezenet network model.
...
bool readPngImage(const char *filename, float *&imageT, size_t *imageDims) {
    ...
    // normalization specific to squeezenet model
    float mean = 127.5;
    float scale = 255;
...
/// initialize.
GLOW_MEM_ALIGN(SQUEEZENET_MEM_ALIGN)
uint8_t constantWeight[SQUEEZENET_CONSTANT_MEM_SIZE] = {
#include "squeezenet.weights.txt"
};

/// Statically allocate memory for mutable weights (model input/output data).
GLOW_MEM_ALIGN(SQUEEZENET_MEM_ALIGN)
uint8_t mutableWeight[SQUEEZENET_MUTABLE_MEM_SIZE];

/// Statically allocate memory for activations (model intermediate results).
GLOW_MEM_ALIGN(SQUEEZENET_MEM_ALIGN)
uint8_t activations[SQUEEZENET_ACTIVATIONS_MEM_SIZE];

/// Bundle input data absolute address.
uint8_t *inputAddr = GLOW_GET_ADDR(mutableWeight, SQUEEZENET_Placeholder);

/// Bundle output data absolute address.
uint8_t *outputAddr = GLOW_GET_ADDR(mutableWeight, SQUEEZENET_softmax_tensor__1);
...
void ShowTops(const char *file_name, milliseconds duration) {
    ...
    // Original Tensorflow squeezenet was trained with 1001 image classification
    int real_idx[5];
    for (int i = 0; i < 5; i++) {
...
int main(int argc, char **argv) {
    ...
    for (unsigned n = 0; n < numImages; n++) {
        ...
        int errCode = squeezenet(constantWeight, mutableWeight, activations);
