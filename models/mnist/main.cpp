...
/// This example loads and runs the compiled mnist network model.
...
/// initialize.
GLOW_MEM_ALIGN(MNIST_MEM_ALIGN)
uint8_t constantWeight[MNIST_CONSTANT_MEM_SIZE] = {
#include "mnist.weights.txt"
};

/// Statically allocate memory for mutable weights (model input/output data).
GLOW_MEM_ALIGN(MNIST_MEM_ALIGN)
uint8_t mutableWeight[MNIST_MUTABLE_MEM_SIZE];

/// Statically allocate memory for activations (model intermediate results).
GLOW_MEM_ALIGN(MNIST_MEM_ALIGN)
uint8_t activations[MNIST_ACTIVATIONS_MEM_SIZE];

/// Bundle input data absolute address.
uint8_t *inputAddr = GLOW_GET_ADDR(mutableWeight, MNIST_data_0);

/// Bundle output data absolute address.
uint8_t *outputAddr = GLOW_GET_ADDR(mutableWeight, MNIST_softmax_1);
    ...
    for (unsigned n = 0; n < numImages; n++) {
        ...
        for (unsigned y = 0; y < dims[1]; y++) {
            for (unsigned x = 0; x < dims[0]; x++) {
                inputT[getXYZ(inputDims, 0, x, y)] = imageT[getXYZ(dims, x, y, 0)];
            }
        }
        ...
        int errCode = mnist(constantWeight, mutableWeight, activations);
